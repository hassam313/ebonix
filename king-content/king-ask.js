...
// ══════════════════════════════════════════════════════════════════════════════
// EBONIX COIN SYSTEM — Global state + UI helpers
// ══════════════════════════════════════════════════════════════════════════════

var ebonixCoins = 0;

function ebonixRefreshCoinDisplay(newBalance) {
    ebonixCoins = newBalance;
    var el = document.getElementById('ebonix-coin-count');
    if (el) el.textContent = newBalance.toLocaleString() + ' coins';
    if (newBalance < 2000 && newBalance > 0) {
        ebonixShowLowCoinWarning(newBalance);
    }
}

function ebonixShowLowCoinWarning(balance) {
    if (document.getElementById('ebonix-low-coin-bar')) return;
    var bar = document.createElement('div');
    bar.id = 'ebonix-low-coin-bar';
    bar.style.cssText = 'position:fixed;bottom:0;left:0;right:0;background:#b45309;color:#fff;text-align:center;padding:10px 16px;z-index:9999;font-size:13px;';
    var link = (typeof qa_root !== 'undefined') ? qa_root + 'membership' : '/membership';
    bar.innerHTML = '&#9888;&#65039; Running low on coins (' + balance.toLocaleString() + ' left). <a href="' + link + '#topup" style="color:#fef3c7;font-weight:600;text-decoration:underline">Top Up Coins</a>';
    document.body.appendChild(bar);
}

function ebonixShowInsufficientCoinsModal(data) {
    var existing = document.getElementById('ebonix-coins-modal');
    if (existing) existing.remove();
    var membershipUrl = (typeof qa_root !== 'undefined') ? qa_root + 'membership' : '/membership';
    var isOnFree = !data.plan || data.plan === 0;
    var html = '<div id="ebonix-coins-modal" style="position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:10000;display:flex;align-items:center;justify-content:center;">'
        + '<div style="background:#1a1a2e;border:1px solid #3a3a5c;border-radius:16px;padding:32px;max-width:420px;width:90%;text-align:center;">'
        + '<div style="font-size:40px;margin-bottom:12px;">🪙</div>'
        + '<h2 style="color:#fff;font-size:22px;margin:0 0 8px;">You\'re out of Coins</h2>'
        + '<p style="color:#a0a0c0;font-size:14px;margin:0 0 20px;">You need <strong style="color:#fff">' + (data.coins_needed || 0).toLocaleString() + ' coins</strong> but only have <strong style="color:#fff">' + (data.coins_balance || 0).toLocaleString() + '</strong>.</p>'
        + '<div style="display:flex;gap:12px;flex-direction:column;">'
        + '<a href="' + membershipUrl + '#topup" style="display:block;background:#7c3aed;color:#fff;padding:12px 20px;border-radius:10px;font-weight:600;text-decoration:none;">Top Up Coins &rarr;</a>'
        + (isOnFree ? '<a href="' + membershipUrl + '" style="display:block;background:#0f172a;border:1px solid #3a3a5c;color:#a0a0c0;padding:12px 20px;border-radius:10px;font-weight:600;text-decoration:none;">Go Flex &mdash; $29/mo</a>' : '')
        + '<button onclick="document.getElementById(\'ebonix-coins-modal\').remove()" style="background:none;border:none;color:#666;cursor:pointer;font-size:13px;padding:8px;">Cancel</button>'
        + '</div></div></div>';
    document.body.insertAdjacentHTML('beforeend', html);
}

function ebonixShowUpgradeModal(feature) {
    var existing = document.getElementById('ebonix-upgrade-modal-coin');
    if (existing) existing.remove();
    var membershipUrl = (typeof qa_root !== 'undefined') ? qa_root + 'membership' : '/membership';
    var featureLabel = feature === 'twin' ? 'AI Twin' : (feature === 'video' ? 'AI Video' : 'this feature');
    var html = '<div id="ebonix-upgrade-modal-coin" style="position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:10000;display:flex;align-items:center;justify-content:center;">'
        + '<div style="background:#1a1a2e;border:1px solid #7c3aed;border-radius:16px;padding:32px;max-width:420px;width:90%;text-align:center;">'
        + '<div style="font-size:40px;margin-bottom:12px;">✨</div>'
        + '<h2 style="color:#fff;font-size:22px;margin:0 0 8px;">Unlock ' + featureLabel + '</h2>'
        + '<p style="color:#a0a0c0;font-size:14px;margin:0 0 20px;">This feature is part of the Flex plan. Get 10,000 coins/month plus full creative access.</p>'
        + '<ul style="text-align:left;color:#c0c0e0;font-size:13px;margin:0 0 20px;padding-left:20px;">'
        + '<li>10,000 Coins every month</li><li>AI Image, Video, and Twin</li><li>Save and reuse your looks</li></ul>'
        + '<a href="' + membershipUrl + '" style="display:block;background:#7c3aed;color:#fff;padding:12px 20px;border-radius:10px;font-weight:600;text-decoration:none;margin-bottom:8px;">Get Flex &mdash; $29/mo &rarr;</a>'
        + '<button onclick="document.getElementById(\'ebonix-upgrade-modal-coin\').remove()" style="background:none;border:none;color:#666;cursor:pointer;font-size:13px;padding:8px;">Maybe later</button>'
        + '</div></div>';
    document.body.insertAdjacentHTML('beforeend', html);
}

// ── Handle coin-aware error codes returned from generation handlers ────────────
function ebonixHandleCoinError(msg, jsonData) {
    if (msg === 'insufficient_coins') {
        ebonixShowInsufficientCoinsModal(jsonData || {});
        return true;
    }
    if (msg === 'upgrade_required') {
        ebonixShowUpgradeModal((jsonData && jsonData.feature) ? jsonData.feature : 'this feature');
        return true;
    }
    if (msg === 'free_plan_no_twin') {
        ebonixShowUpgradeModal('twin');
        return true;
    }
    if (msg === 'free_plan_no_video') {
        ebonixShowUpgradeModal('video');
        return true;
    }
    return false;
}

// ── Load coin balance on page load ────────────────────────────────────────────
document.addEventListener('DOMContentLoaded', function() {
    var ajaxBase = (typeof qa_root !== 'undefined') ? qa_root + 'king-include/king-ajax.php' : null;
    if (!ajaxBase) return;
    var fd = new FormData();
    fd.append('qa_operation', 'getcoins');
    fd.append('qa_request',   '');
    fd.append('qa_root',      typeof qa_root !== 'undefined' ? qa_root : '');
    var xhr = new XMLHttpRequest();
    xhr.open('POST', ajaxBase);
    xhr.onload = function() {
        try {
            var r = JSON.parse(xhr.responseText.trim());
            if (r.status === 'ok') ebonixRefreshCoinDisplay(r.coins);
        } catch(e) {}
    };
    xhr.send(fd);
});
...
// ══════════════════════════════════════════════════════════════════════════════
    ...
    qa_ajax_post("aivideo", params, function(lines) {
      ...
      if (success) {
        try {
          ...
          // Update coin display on video success
          if (typeof response.coins_remaining !== 'undefined') {
              ebonixRefreshCoinDisplay(response.coins_remaining);
          }
          } else {
            var vidErrMsg = response.message || 'Video generation failed';
            if (!ebonixHandleCoinError(vidErrMsg, response)) {
                console.error('❌ Response success=false:', vidErrMsg);
                if (aierror) {
                    aierror.style.display = 'block';
                    aierror.innerHTML = '❌ ' + vidErrMsg;
                }
            }
          }
        } catch (parseError) {
        ...
        try {
          var errorResponse = JSON.parse(lines[1]);
          var vidErrMsg2 = errorResponse.message || 'Video generation failed';
          if (!ebonixHandleCoinError(vidErrMsg2, errorResponse)) {
              if (aierror) {
                  aierror.style.display = 'block';
                  aierror.innerHTML = '❌ ' + vidErrMsg2;
              }
          }
        } catch (e) {
          if (aierror) {
  ...
  function aipromter(elem) {
    ...
    // Include active style chip context
    if (typeof ebxGetActiveChipContext === 'function') {
        var ctx = ebxGetActiveChipContext();
        if (ctx) params.style_context = ctx;
    }
    // Include active scene context
    if (typeof _ebxActiveScene !== 'undefined' && _ebxActiveScene && _ebxActiveScene.prompt) {
        params.style_context = (params.style_context ? params.style_context + ', ' : '') + _ebxActiveScene.prompt;
    }
  ...
  if (results) {
      ...
      if (html) {
          ...
          results.scrollIntoView({ behavior: 'smooth', block: 'start' });
            ...
            var errMsg = response.message || 'Generation failed. Please try again.';
            if (!ebonixHandleCoinError(errMsg, response)) {
                showErr(errMsg);
            }
        }

        // ── Update coin display on success ────────────────────────────────
        if (response.success && typeof response.coins_remaining !== 'undefined') {
            ebonixRefreshCoinDisplay(response.coins_remaining);
            // Fire event so submitai page can show identity badge, save prompt, etc.
            document.dispatchEvent(new CustomEvent('ebx:generation:success', { detail: response }));
        }
        ...
        var errPayload = null;
        try {
            errPayload = JSON.parse(json);
            if (errPayload && errPayload.message) msg = errPayload.message;
        } catch (e) {
            if (json) msg = json;
        }
        if (!ebonixHandleCoinError(msg, errPayload)) {
            showErr(msg);
        }
