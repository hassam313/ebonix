	...
	if ($postformat === 'V') {
		...
		if (is_numeric($extra)) {
			...
			if ($video && isset($video['furl'])) {
				// furl is already HTML-escaped by king_get_uploads(); decode first to avoid double-escape
				$vid_src = htmlspecialchars_decode($video['furl'], ENT_QUOTES);
				$html .= '<video class="simple-post-video" muted loop playsinline preload="metadata" poster="">';
				$html .= '<source src="' . qa_html($vid_src) . '" type="video/mp4">';
				$html .= '</video>';
			}
		} else {
...
function kingai_check()
{
	...
	// Hardcoded fallback image limits (used when admin leaves plan_X_lmt at 0)
	$img_defaults = [1 => 60, 2 => 160, 3 => 350];

	$pl = null;
	if ($mp > 0) {
		$raw_pl = (INT)qa_opt('plan_' . $mp . '_lmt');
		$pl = ($raw_pl > 0) ? $raw_pl : ($img_defaults[$mp] ?? null);
	} elseif (qa_opt('ulimits')) {
		$raw_pl = (INT)qa_opt('ulimit');
		if ($raw_pl > 0) $pl = $raw_pl;
	}

	// No limit configured → allow (unlimited for this plan)
