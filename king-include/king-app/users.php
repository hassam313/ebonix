    ...
    foreach ( $aiposts as $aipost ) {
		...
		$extras = @unserialize( $extraz );
			...
			if ( $type === 'aimg' && is_array( $extras ) ) {
				...
				if ( $vid && !empty($vid['furl']) ) {
					// furl is already HTML-escaped inside king_get_uploads(); use raw content for src
					$vid_src = htmlspecialchars_decode($vid['furl'], ENT_QUOTES);
					$html .= '<video class="king-aivid" id="video_' . qa_html($postid) . '" autoplay loop muted playsinline>';
					$html .= '<source src="' . qa_html($vid_src) . '" type="video/mp4"></source>';
					$html .= '</video>';
					...
					// Video file unavailable (download failed at generation time)
					$html .= '<div class="king-aivid-unavailable">';
					$html .= '<i class="fa-solid fa-video-slash"></i>';
					$html .= '<span>Video unavailable</span>';
					$html .= '</div>';
				}
			}
		...
		if ( isset( $aipost['pcontent'])) {
			...
			if ( isset( $imageid ) && $imageid ) {
				...
				if ( $imageurl && isset( $imageurl['furl'] ) ) {
