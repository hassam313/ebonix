...
// Correct Fal submit returning response_url from submit body — used by Ebonix models
if (!function_exists('king_fal_ebonix_submit')) {
    function king_fal_ebonix_submit($endpoint, $payload, $fal_api_key) {
        ...
        $data = json_decode($response, true);
        $rid  = $data['request_id'] ?? '';
        if (empty($rid)) return ['error' => 'Fal returned no request_id: ' . substr((string)$response, 0, 200)];
        $response_url = $data['response_url'] ?? ('https://queue.fal.run/' . $endpoint . '/requests/' . $rid);
        return ['request_id' => $rid, 'response_url' => $response_url];
    }
}

// Poll response_url directly — never constructs /status (returns 405)
if (!function_exists('king_fal_ebonix_poll')) {
    function king_fal_ebonix_poll($response_url, $fal_api_key, $max_attempts = 90, $sleep_sec = 5) {
        ...
        for ($i = 0; $i < $max_attempts; $i++) {
            ...
            $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
            curl_close($ch);
            error_log("king_fal_ebonix_poll: attempt={$i} HTTP={$code}");

            if ($code === 200 && !empty($raw)) {
                $body = json_decode($raw, true);

                // Fal queue may wrap output: {"output": {"images": [...]}} or return it directly
                $output = (isset($body['output']) && is_array($body['output'])) ? $body['output'] : $body;

                // Extract image URLs — handle both array-of-objects and array-of-strings
                $images = $output['images'] ?? $output['image'] ?? [];
                ...
                foreach ($images as $img) {
                    if (is_string($img) && filter_var($img, FILTER_VALIDATE_URL)) {
                        ...
                        $u = $img['url'] ?? $img['image_url'] ?? $img['src'] ?? '';
                        if ($u && filter_var($u, FILTER_VALIDATE_URL)) $urls[] = $u;
                    }
                }

                if (!empty($urls)) {
                    error_log("king_fal_ebonix_poll: DONE " . count($urls) . " image(s) url0=" . $urls[0]);
                    return ['urls' => $urls];
                }

                // Check explicit failure
                $state = strtoupper($body['status'] ?? $output['status'] ?? '');
                if ($state === 'FAILED') {
                    $err = $body['error'] ?? $body['detail'] ?? $output['error'] ?? 'unknown';
                    return ['error' => 'Fal generation failed: ' . $err];
                }

                // Still processing — log raw for debugging on first in-progress 200
                if ($i === 0) {
                    error_log("king_fal_ebonix_poll: 200 but no images yet, body=" . substr($raw, 0, 300));
                }
                continue;
            }
...
// ============================================================
// EBONIX BLACK REPRESENTATION INJECTION
// Applies to all direct-API models that bypass the gateway.
// banana and fluxkon_selfie are excluded — they handle identity
// through their own prompt logic.
// ============================================================
if (!function_exists('ebonix_inject_black_representation')) {
    function ebonix_inject_black_representation($prompt, $model) {
        // Models that have their own representation logic
        if (in_array($model, ['banana', 'fluxkon_selfie'], true)) return $prompt;
        ...
        $person_kw = [
            ...
            'model','portrait','face','queen','king','sister','brother',
            'mother','father','couple','friends','family',
        ];
        $pl = strtolower($prompt);
        $has_person = false;
        foreach ($person_kw as $kw) {
            if (strpos($pl, $kw) !== false) { $has_person = true; break; }
        }
        if (!$has_person) return $prompt;

        // Don't double-inject if representation already present
        if (stripos($prompt, 'melanin') !== false || stripos($prompt, 'dark skin') !== false) return $prompt;

        // DALL-E (de, de3) uses OpenAI which rejects explicit racial framing —
        // use the safer keyword-replacement approach instead of a hard prefix.
        if (in_array($model, ['de', 'de3'], true)) {
            $replacements = [
                'beautiful '  => 'beautiful dark-skinned ',
                'Beautiful '  => 'Beautiful dark-skinned ',
                'handsome '   => 'handsome dark-skinned ',
                'Handsome '   => 'Handsome dark-skinned ',
                'cute '       => 'cute dark-skinned ',
                'Cute '       => 'Cute dark-skinned ',
                'pretty '     => 'pretty dark-skinned ',
                'Pretty '     => 'Pretty dark-skinned ',
                'gorgeous '   => 'gorgeous dark-skinned ',
                'Gorgeous '   => 'Gorgeous dark-skinned ',
            ];
            $modified = strtr($prompt, $replacements);
            // If no adjective was replaced, inject directly before a subject keyword
            if ($modified === $prompt) {
                foreach (['woman','man','girl','boy','person','model','portrait'] as $kw) {
                    if (stripos($modified, $kw) !== false) {
                        $modified = str_ireplace($kw, "dark-skinned {$kw}", $modified);
                        break;
                    }
                }
            }
        ...
        // All other non-gateway models (imagen4, luma_img, decart_img, kingstudio)
        // can safely accept a more descriptive representation prefix.
        $prefix = 'Dark-skinned person with rich warm skin tones, authentic natural hair, strong beautiful features. ';
        $suffix = ', warm rich skin tones, natural diverse hair, authentic features, no whitewashing';
...
// Whitelist allowed model keys — includes old keys so regenerate on legacy posts still works
$allowed_models = [
    // Legacy keys (kept so old Regenerate buttons don't return "Invalid model")
    'de', 'de3', 'banana', 'imagen4', 'flux', 'fluxs', 'fluxkon',
    'flux_pro', 'sdream', 'decart_img', 'luma_img', 'fluxkon_selfie',
    // Active Ebonix Fal AI models
    'ebonix_10', 'ebonix_20', 'ebonix_classic',
    'ebonix_advanced', 'ebonix_flash', 'ebonix_pro', 'ebonix_studio',
];
if (!in_array($aiselect, $allowed_models, true)) {
    echo "QA_AJAX_RESPONSE\n0\n" . json_encode(['success' => false, 'message' => 'Invalid model.']) . "\n";
    exit;
}

// Redirect all legacy model keys to their Ebonix equivalents.
// This keeps Regenerate working on old posts without any other changes.
$_ebonix_redirects = [
    'banana'     => 'ebonix_20',
    'imagen4'    => 'ebonix_pro',
    'de'         => 'ebonix_flash',
    ...
    'flux'       => 'ebonix_flash',
    'fluxs'      => 'ebonix_flash',
    'fluxkon'    => 'ebonix_studio',
    'flux_pro'   => 'ebonix_pro',
    'sdream'     => 'ebonix_20',
    'decart_img' => 'ebonix_10',
    'luma_img'   => 'ebonix_20',
];
if (isset($_ebonix_redirects[$aiselect])) {
    error_log("aigenerate: redirecting legacy {$aiselect} → {$_ebonix_redirects[$aiselect]}");
    $aiselect = $_ebonix_redirects[$aiselect];
}

// Whitelist aspect ratio / size format
if (!preg_match('/^\d{1,5}x\d{1,5}$/', $imsize) && !preg_match('/^\d{1,2}:\d{1,2}$/', $imsize)) {
    $imsize = '1024x1024';
}
...
// ── Plan + Coin enforcement — cost calculated for ALL users; enforcement for non-admins only ──
$is_admin       = $is_logged_in && qa_get_logged_in_level() >= QA_USER_LEVEL_ADMIN;
$img_tier_key   = 'standard';
$img_tier_label = 'Standard Photo';
$img_chosen_model = $aiselect;
$img_total_cost = 0;
$img_auto_selected = false;
...
if ($is_logged_in) {
    ...
    if ($auto_mode === '1') {
        ...
        if (array_filter($premium_kw, function($k) use ($prompt_lower) { return strpos($prompt_lower, $k) !== false; })) {
            $img_tier_key = 'premium';
        } elseif (array_filter($beauty_kw, function($k) use ($prompt_lower) { return strpos($prompt_lower, $k) !== false; })) {
            $img_tier_key = 'beauty';
        } elseif (array_filter($budget_kw, function($k) use ($prompt_lower) { return strpos($prompt_lower, $k) !== false; })) {
            $img_tier_key = 'standard';
        } else {
            $img_tier_key = 'enhanced';
        }
        $img_auto_selected = true;
    } else {
        // Manual: use posted tier or infer from model key
        $posted_tier = trim((string)(isset($_POST['tier']) ? $_POST['tier'] : ''));
        if ($posted_tier && array_key_exists($posted_tier, ebonix_get_photo_tiers())) {
            $img_tier_key = $posted_tier;
        } else {
            $img_tier_key = ebonix_model_to_photo_tier((string)$aiselect);
        }
        $img_auto_selected = false;
    }

    $img_tiers      = ebonix_get_photo_tiers();
    $img_tier_data  = $img_tiers[$img_tier_key] ?? $img_tiers['standard'];
    $img_tier_label = $img_tier_data['label'];
    $img_base_cost  = $img_tier_data['coins'];
    ...
    if (!$is_admin) {
        // Free plan can only use standard tier
        if ($user_plan_now === 0 && $img_tier_key !== 'standard') {
            echo "QA_AJAX_RESPONSE\n0\n" . json_encode([
                'success' => false,
                'message' => 'upgrade_required',
                'feature' => 'premium_photo',
                'msg'     => 'Premium quality tiers require the Flex plan.',
            ]) . "\n";
            exit;
        }

        if (!ebonix_has_coins($userid, $img_total_cost)) {
            echo "QA_AJAX_RESPONSE\n0\n" . json_encode([
                'success'       => false,
                'message'       => 'insufficient_coins',
                'coins_needed'  => $img_total_cost,
                'coins_balance' => ebonix_get_coins($userid),
                'shortfall'     => $img_total_cost - ebonix_get_coins($userid),
            ]) . "\n";
            exit;
        }
    }
...
// Representation-enhanced prompt used by all direct-API models that bypass the gateway
$rep_effective_input = ebonix_inject_black_representation($effective_input, $aiselect);
...
if (!$gemini_processed && empty($image_urls)) {
    ...
    // ──────────────────────────────────────────────────────────────────────
        ...
        if (empty($API_KEY)) {
            ...
            $instance     = ['prompt' => $rep_effective_input];
    ...
    // ──────────────────────────────────────────────────────────────────────
        ...
        if (empty($API_KEY)) {
            ...
            $decart_payload = ['prompt' => $rep_effective_input];
            if (!empty($npvalue)) $decart_payload['negative_prompt'] = $npvalue;

            $ref_tmp_path = '';
            if ($ref_binary !== false) {
                $ext_map      = ['image/jpeg' => 'jpg', 'image/png' => 'png', 'image/webp' => 'webp', 'image/gif' => 'gif'];
                $te           = $ext_map[$ref_mime] ?? 'jpg';
                $ref_tmp_path = tempnam(sys_get_temp_dir(), 'decart_ref_') . '.' . $te;
                file_put_contents($ref_tmp_path, $ref_binary);
                $decart_payload['image_url'] = rtrim(qa_opt('site_url'), '/') . '/king-include/uploads/' . date('Y') . '/' . date('m') . '/' . basename($ref_tmp_path);
            }

            $ch = curl_init('https://api.decart.ai/v1/images/generations');
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_HTTPHEADER, [
            ...
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($decart_payload));
            curl_setopt($ch, CURLOPT_TIMEOUT, 120);
            $decart_raw  = curl_exec($ch);
            $http_code   = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
            $decart_cerr = curl_error($ch);
            curl_close($ch);
            if (!empty($ref_tmp_path) && file_exists($ref_tmp_path)) @unlink($ref_tmp_path);
            error_log("Decart: HTTP={$http_code} body=" . substr((string)$decart_raw, 0, 400));

            if (!empty($decart_cerr)) {
                $error = 'Decart CURL error: ' . $decart_cerr;
            } elseif ($http_code !== 200 && $http_code !== 201) {
                $je    = @json_decode($decart_raw, true);
                $error = 'Decart HTTP ' . $http_code . ': ' . ($je['message'] ?? $je['error']['message'] ?? $je['error'] ?? substr((string)$decart_raw, 0, 200));
            } else {
                $decart_json = @json_decode($decart_raw, true);
                // Handle JSON response with URL(s)
                $decart_url = $decart_json['data'][0]['url']
                    ?? $decart_json['images'][0]['url']
                    ?? $decart_json['image_url']
                    ?? $decart_json['url']
                    ?? ($decart_json['output'][0] ?? '');

                if (!empty($decart_url) && filter_var($decart_url, FILTER_VALIDATE_URL)) {
                    $image_urls[] = $decart_url;
                } else {
                    // Response might be raw binary image bytes
                    $ii = false;
                    if (strlen((string)$decart_raw) > 500) {
                        $tmp_check = tempnam(sys_get_temp_dir(), 'decart_chk_');
                        file_put_contents($tmp_check, $decart_raw);
                        $ii = @getimagesize($tmp_check);
                        @unlink($tmp_check);
                    }
                    if ($ii) {
                        $folder  = 'uploads/' . date('Y') . '/' . date('m') . '/';
                        ...
                        $tmp   = $destDir . 'temp_' . $final;
                        $full  = $destDir . $final;
                        file_put_contents($tmp, $decart_raw);
                        $tr = king_process_local_image($tmp, $folder . $final, true, 600);
                        if (copy($tmp, $full)) {
                            list($w, $h) = $ii;
                        ...
                        $error = 'Decart returned unrecognised response. Check PHP error log for details.';
                    }
                }
            }
        }
    ...
    // ──────────────────────────────────────────────────────────────────────
        ...
        if (empty($API_KEY)) {
            ...
            $prompt       = (string)$rep_effective_input;
    ...
    // ──────────────────────────────────────────────────────────────────────
    // EBONIX IMAGES — Direct Fal AI (7 branded models)
    // Each model has its own API key stored in admin options.
    // All use Fal async queue: submit → poll response_url (never /status).
    // ──────────────────────────────────────────────────────────────────────
    } elseif (in_array($aiselect, ['ebonix_10','ebonix_20','ebonix_classic','ebonix_advanced','ebonix_flash','ebonix_pro','ebonix_studio'], true)) {

        $ebonix_conf = [
            'ebonix_10'       => ['endpoint' => 'fal-ai/nano-banana',        'key_opt' => 'fal_key_ebonix_10',       'type' => 'nano'],
            'ebonix_20'       => ['endpoint' => 'fal-ai/nano-banana-2',       'key_opt' => 'fal_key_ebonix_20',       'type' => 'nano'],
            'ebonix_classic'  => ['endpoint' => 'fal-ai/nano-banana',         'key_opt' => 'fal_key_ebonix_classic',  'type' => 'nano'],
            'ebonix_advanced' => ['endpoint' => 'fal-ai/flux-2-pro',          'key_opt' => 'fal_key_ebonix_advanced', 'type' => 'flux'],
            'ebonix_flash'    => ['endpoint' => 'fal-ai/flux/dev',            'key_opt' => 'fal_key_ebonix_flash',    'type' => 'flux'],
            'ebonix_pro'      => ['endpoint' => 'fal-ai/flux-pro/v1.1-ultra', 'key_opt' => 'fal_key_ebonix_pro',      'type' => 'flux_pro'],
            'ebonix_studio'   => ['endpoint' => 'fal-ai/flux-pro/kontext',    'key_opt' => 'fal_key_ebonix_studio',   'type' => 'kontext'],
        ];

        $conf        = $ebonix_conf[$aiselect];
        $fal_api_key = trim((string)qa_opt($conf['key_opt']));
        $endpoint    = $conf['endpoint'];
        $model_type  = $conf['type'];

        if (empty($fal_api_key)) {
            $error = 'API key for ' . $aiselect . ' not configured. Add it in Admin → Ebonix Images.';
        } else {
            // ── Derive aspect ratio from imsize ──────────────────────────────
            $eb_aspect = '1:1';
            if (preg_match('/^(\d+)x(\d+)$/', $imsize, $ms)) {
                $ew = (int)$ms[1]; $eh = (int)$ms[2];
                if ($ew === $eh)        $eb_aspect = '1:1';
                elseif ($ew > $eh)      $eb_aspect = ($ew / $eh >= 1.6) ? '16:9' : '4:3';
                else                   $eb_aspect = ($eh / $ew >= 1.6) ? '9:16' : '3:4';
            } elseif (preg_match('/^\d+:\d+$/', $imsize)) {
                $eb_aspect = $imsize;
            }

            // ── Upload ref image to Fal storage when available ───────────────
            $eb_fal_image_url = '';
            if ($ref_binary !== false) {
                $eb_resized        = king_resize_binary($ref_binary, $ref_mime, 1536);
                $eb_fal_image_url  = king_fal_upload_storage($eb_resized, $ref_mime, $fal_api_key);
                if (empty($eb_fal_image_url) && !empty($ref_pub_url) && filter_var($ref_pub_url, FILTER_VALIDATE_URL)) {
                    $eb_fal_image_url = $ref_pub_url;
                }
            }

            // ── Build payload per model type ─────────────────────────────────
            if ($model_type === 'nano') {
            ...
            } elseif ($model_type === 'flux') {
                // FLUX Dev / FLUX.2 Pro: text-to-image (optional image reference)
                $eb_w = 1024; $eb_h = 1024;
                if (preg_match('/^(\d+)x(\d+)$/', $imsize, $ms2)) {
                    $eb_w = (int)$ms2[1]; $eb_h = (int)$ms2[2];
                }
                $eb_payload = [
                    'prompt'                => $rep_effective_input,
                    'image_size'            => ['width' => $eb_w, 'height' => $eb_h],
                    'num_images'            => 1,
                ...
                // FLUX Kontext (Studio):
                //   • With ref image  → fal-ai/flux-pro/kontext          (image editing)
                //   • Without image   → fal-ai/flux-pro/kontext/text-to-image (text-to-image)
            ...
            error_log("{$aiselect}: endpoint={$endpoint} type={$model_type} has_ref=" . (!empty($eb_fal_image_url) ? 'yes' : 'no'));

            // ── Submit to Fal queue (only if payload built successfully) ────────
            if (empty($error)) {
                $eb_submit = king_fal_ebonix_submit($endpoint, $eb_payload, $fal_api_key);
                if (!empty($eb_submit['error'])) {
                    $error = $eb_submit['error'];
                    error_log("{$aiselect} submit error: {$error}");
                } else {
                    error_log("{$aiselect}: request_id={$eb_submit['request_id']}");

                    // ── Poll response_url — 90 × 5s = 450s max ────────────────
                    $eb_poll = king_fal_ebonix_poll($eb_submit['response_url'], $fal_api_key, 90, 5);
                    if (!empty($eb_poll['error'])) {
                        $error = $eb_poll['error'];
                        error_log("{$aiselect} poll error: {$error}");
                    } else {
                        $image_urls = $eb_poll['urls'];
                        error_log("✅ {$aiselect} SUCCESS: " . count($image_urls) . " image(s)");
                    }
                }
            }
        }
    ...
    // ──────────────────────────────────────────────────────────────────────
        ...
        if (empty($sdapi)) {
            ...
            $request_data = [
                'prompt' => $rep_effective_input . ($style_preset ? ', ' . $style_preset : ''),
                'size'   => (int)$imagen,
                'steps'  => (int)$aisteps,
                'aisize' => $imsize,
                'model'  => $aiselect,
...
$i2i_models_for_meta = ['fluxkon', 'sdream', 'banana', 'decart_img', 'luma_img', 'imagen4', 'de', 'fluxkon_selfie',
    'ebonix_10', 'ebonix_20', 'ebonix_classic', 'ebonix_advanced', 'ebonix_flash', 'ebonix_pro', 'ebonix_studio'];
...
// ── Deduct coins on confirmed image success (all logged-in users) ─────────────
$img_new_balance = 0;
if ($is_logged_in && $img_total_cost > 0) {
    $img_new_balance = ebonix_deduct_coins(
        $userid, $img_total_cost,
        'image_' . $img_tier_key,
        (string)$aiselect,
        $postid
    );
} elseif ($is_logged_in) {
    $img_new_balance = ebonix_get_coins($userid);
}

error_log("✅ aigenerate complete: postid={$postid} model={$aiselect} tier={$img_tier_key} coins={$img_total_cost} remaining={$img_new_balance}");

echo "QA_AJAX_RESPONSE\n1\n" . json_encode([
    'success'          => true,
    'postid'           => $postid,
    'model_label'      => $img_tier_label,
    'auto_selected'    => $img_auto_selected,
    'coins_deducted'   => $img_total_cost,
    'coins_remaining'  => $img_new_balance,
]) . "\n";
